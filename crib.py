...
objs.append(Sphere( Vector(-4,0,-10), 2.0, Vector(0,255,0)))   # center, radius, color(=RGB)
...
for x in range(w):  # loop over all x values for our image
    print (x)   # provide some feedback to the user about our progress
    for y in range(h):  # loop over all y values
        ray = Ray( cameraPos, (Vector(x/50.0-5,y/50.0-5,0)-cameraPos).normal())
        col = trace(ray, objs, lightSource, 1)
        img.putpixel((x,h-1-y),gammaCorrection(col,GAMMA_CORRECTION))
